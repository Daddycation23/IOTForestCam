 ...
 *
 * All LoRa packets share a 3-byte header: magic(0xFC), version(0x01), type.
...
static constexpr uint8_t LORA_PKT_VERSION = 0x01;
...
inline uint8_t getLoRaPacketType(const uint8_t* buf, uint8_t len) {
    ...
    if (buf[1] != LORA_PKT_VERSION) return LORA_PKT_UNKNOWN;
    return buf[2];
}
