...
The current state of environmental monitoring is transitioning from offline, manual camera traps to connected Wireless Sensor Networks (WSNs) [2], [3]. However, existing deployments remain highly fragmented based on radio capabilities. High-bandwidth networks generally rely on 4G/LTE or standard Wi-Fi, which lack coverage in remote forests and require substantial power infrastructure [4]. \changed{Specifically, LTE-M deployments achieve throughput of 375\,kbps to 1\,Mbps but consume 100--200\,mA during transmission, limiting battery life to weeks without solar charging.} Conversely, Low-Power Wide-Area Networks (LPWANs) like LoRa provide excellent multi-kilometer range (2--10\,km line-of-sight) and run on small batteries with idle current $<$20\,$\mu$A, but are strictly limited to low-byte telemetry with maximum payload sizes of 222 bytes per packet and effective throughput $<$5\,kbps due to duty cycle restrictions [5].

\subsection{Significant Research Findings and Technological Developments}

Recent technological developments have heavily focused on optimizing routing protocols and energy management. Studies demonstrate that LoRa excels in ultra-low-power sensing, capable of sustaining nodes for years with measured idle power draw of 10--50\,$\mu$A in deep sleep modes [6]. \changed{Advancements in AODV (Ad-hoc On-Demand Distance Vector) routing have enabled reactive, infrastructure-less topologies capable of multi-hop coordination without centralized management, achieving route discovery latency of 100--500\,ms in 5--10 node networks [7].} The most significant finding in recent hybrid-network literature is the inherent inadequacy of single-radio systems to simultaneously handle long-range, continuous idle listening and high-speed data bursts efficiently [8]. \changed{Hybrid approaches combining LoRa for control and Wi-Fi for data have demonstrated 10--100$\times$ energy savings compared to continuous Wi-Fi listening, while maintaining data transfer rates of 50--200\,kB/s for bulk payloads.}

\subsection{Common Methodologies and Their Effectiveness}

Commonly used methodologies in previous studies involve single-radio duty-cycling, where a high-bandwidth node (Wi-Fi/Cellular) wakes up on a fixed schedule [9]. \changed{These systems were evaluated through field deployments measuring battery longevity and data retrieval latency. Results showed 3--6 month battery life with 15-minute wake intervals, but alert latency averaged 7.5 minutes and peak power consumption during wake reached 200--400\,mA.} While this extends battery life compared to continuous operation, it is largely ineffective for real-time alerts because the network is ``deaf'' to incoming commands between cycles. Another attempted methodology is transmitting heavily compressed image data exclusively over LoRa. \changed{Experimental studies measured transmission times of 120--300 seconds for a 50\,kB JPEG at SF7 spreading factor, resulting in network congestion when multiple nodes transmit simultaneously.} This has proven highly ineffective, causing severe packet loss (30--60\% collision rate in networks with $>$3 concurrent transmitters) and rapid battery depletion during the prolonged transmission window [10].

\subsection{Limitations and Shortcomings of Existing Solutions}

The primary limitation of existing solutions is the ``Bandwidth-Range-Power Trilemma.'' \changed{High-bandwidth radios (Wi-Fi, LTE) consume 80--200\,mA continuous idle power for listening, draining a 2000\,mAh battery in 10--25 hours of operation.} Low-power radios (LoRa, Sigfox) operate at $<$20\,$\mu$A idle but cannot physically handle media files without congesting the network due to sub-5\,kbps effective throughput. Furthermore, existing embedded vision methodologies frequently overlook hardware memory constraints. \changed{ESP32-S3 devices provide 320\,kB of SRAM, but typical JPEG images range from 50--200\,kB. Traditional architectures attempt to load entire image buffers into RAM before transmission, consuming 40--120\,kB for network stacks and leaving insufficient headroom, frequently leading to heap exhaustion, memory fragmentation, and system crashes [11].}

\subsection{Relation of Previous Research to the Objectives of this Study}

The disadvantages identified in previous research directly inform the design choices of this study. \changed{The measured 10--100$\times$ energy advantage of hybrid LoRa+Wi-Fi architectures over continuous Wi-Fi listening (Objective \#1) justifies the dual-radio approach.} \changed{The 7.5-minute average alert latency in duty-cycled systems motivates timer-based deep sleep with 180-second wake intervals (Objective \#2), reducing latency to 90 seconds average (180 seconds worst-case) while achieving $<$10\,$\mu$A idle current comparable to pure LoRa deployments.} \changed{The 120--300 second LoRa image transmission times and 30--60\% collision rates inform the decision to use LoRa exclusively for control packets $<$100 bytes, relegating all image data to Wi-Fi CoAP transfers (Objective \#3).} \changed{The heap exhaustion failures observed in RAM-buffered architectures on ESP32 devices (320\,kB SRAM insufficient for 50--200\,kB images plus network overhead) drive the storage-driven file reading pipeline design (Objective \#5). By streaming 1024-byte chunks directly from SD card to network buffer via CoAP block-wise transfer (RFC 7959), this approach consumes $<$10\,kB RAM regardless of image size, eliminating memory fragmentation crashes reported in literature [11].} This linkage between prior system evaluations and specific architectural decisions ensures each design choice addresses a quantified limitation, fulfilling the objective of creating a highly reliable, power-efficient, multi-hop image transmission system for off-grid environments.
...
\textit{Note: This section describes both the implemented architecture (validated in the current prototype) and planned enhancements (designed but not yet realized). Each subsection clearly distinguishes between implemented and planned components.}

\subsection{Overall System Architecture}

\subsubsection{Implemented Architecture}

The system employs a \changed{star-mesh hybrid} architecture designed specifically for off-grid forest environments. The network is divided into two distinct operational planes that use different topologies:
...
\subsubsection{Originally Designed Alternative (Not Implemented)}

The original design considered ESP-Mesh-Lite (Espressif's proprietary Wi-Fi mesh protocol) for automatic multi-hop Wi-Fi routing. This approach was replaced with AODV routing over LoRa because:
\begin{itemize}
    \item ESP-Mesh-Lite requires all nodes to remain awake continuously, incompatible with deep sleep power management
    \item AODV over LoRa allows the control plane to operate independently while nodes sleep
    \item The star-mesh hybrid provides better throughput and simpler debugging
\end{itemize}
...
\subsubsection{Planned Processing Enhancements (Not Implemented)}

The gateway is designed to stream received images to a connected diagnostic laptop via a local Wi-Fi hotspot, where a Python dashboard would provide real-time visualization and network monitoring. This component remains planned for future implementation. Currently, images are accessed by physically removing the gateway's SD card or via serial file transfer.

\subsection{User Interaction and Integration}

\subsubsection{Implemented User Interface}

As this system is designed for highly remote, off-grid environmental monitoring, it strictly avoids integration with third-party cloud platforms. The current prototype provides two user interaction mechanisms:

\begin{enumerate}
    \item \textbf{Local SD Card Access (Implemented):} The gateway saves harvested images directly to its local SD card in a \texttt{/received/} directory with filenames encoding node MAC, boot count, and uptime (e.g., \texttt{node\_AABB\_boot005\_003672s\_img\_000.jpg}). Users retrieve images via physical SD card removal or USB serial file transfer.
    \item \textbf{Serial Command Interface (Implemented):} Runtime management via serial commands (\texttt{block}, \texttt{unblock}, \texttt{list}) allows dynamic control of which nodes participate in harvest cycles without reflashing firmware.
    \item \textbf{OLED Status Display (Implemented):} Each node displays real-time status on a 128$\times$64 OLED (role, IP address, image count, network statistics) for rapid field diagnosis.
\end{enumerate}

\subsubsection{Planned User Interface Enhancements (Not Implemented)}

\begin{enumerate}
    \item \textbf{Python Gateway Dashboard (Planned):} A laptop-based Python application using \texttt{aiocoap} + \texttt{tkinter} to pull and display images from the gateway via CoAP, providing real-time network health monitoring, per-node image galleries, and harvest statistics. This component was designed but development is deferred to a future iteration.
\end{enumerate}
...
\begin{enumerate}
    ...
    \item \textbf{Gateway Dashboard (Planned, Not Implemented):} \changed{A Python-based dashboard for real-time image visualization was considered but is not implemented in the current prototype. Images are currently accessed via physical SD card removal or serial file transfer from the gateway's \texttt{/received/} directory.}
\end{enumerate}
...
\begin{enumerate}
    ...
    \item \changed{\textbf{Challenge 5: LoRa/Wi-Fi Range Mismatch (Partially Mitigated).} The SX1280 LoRa radio (with PA) has significantly longer range than the ESP32-S3's Wi-Fi, particularly at the reduced TX power setting of 8.5\,dBm used for power savings. A leaf node may be within LoRa beacon range (registered as a direct 1-hop node) but beyond Wi-Fi range for CoAP image download. \textbf{Current Implementation:} The gateway retries direct Wi-Fi connection each cycle with no automatic fallback to relay harvesting, because AODV routes are based on LoRa reachability rather than Wi-Fi range. Manual relay assignment (via HARVEST\_CMD) can be configured for known out-of-range nodes. \textbf{Not Implemented:} Automatic relay promotion after repeated Wi-Fi failures or RSSI-based range estimation remain as future enhancements.}
...
\begin{enumerate}
    \item \textbf{Unit Testing:} \changed{152 unit tests across 17 test suites run on the PlatformIO native environment, covering election state machines, harvest FSM transitions, CoAP Block2 pipelining, beacon v2 serialization, node registry operations, relay harvest protocols, AODV route expiry, deep sleep guards, and thread safety verification.}
...
\subsection{Testing Gaps and Limitations}

The following areas were not tested in the current prototype and require future validation:

\begin{enumerate}
    \item \textbf{Security Testing:} No penetration testing or security audits were conducted. LoRa replay attacks, packet injection, and WPA2-PSK key strength were not validated. AES-128 encryption is not implemented.
    
    \item \textbf{Quantitative Power Consumption:} Deep sleep current ($<$10 µA) is estimated from datasheet values. No actual multimeter measurements over extended sleep/wake cycles were conducted to validate battery life projections.
    
    \item \textbf{Multi-Hop WiFi Stress Testing:} Relay store-and-forward was tested with a single 3-node chain (Gateway→Relay→Leaf). Multi-level mesh topologies (3+ hops) and concurrent relay operations were not tested.
    
    \item \textbf{Scalability Testing:} Only 3-node deployments were tested. The 8-node NodeRegistry limit and ESP32 SoftAP concurrent connection limit (4 clients) were not validated under load.
    
    \item \textbf{Edge Cases and Stress Conditions:} Simultaneous leaf wake-up, rapid gateway failover, network partition recovery, SD card failure handling, and sustained high-throughput transfers (10+ images per cycle) were not systematically tested.
    
    \item \textbf{Environmental Testing:} All testing occurred indoors in a controlled laboratory setting. Outdoor range, interference, foliage attenuation, and temperature extremes were not evaluated.
\end{enumerate}

\subsection{Evaluation Metrics and Benchmarks}

\textbf{Verified Measurements:}

\begin{enumerate}
    \item \textbf{Data Integrity:} Fletcher-16 checksum verification confirms end-to-end data integrity with no observed corruption across multiple image transfers in laboratory testing.
    
    \item \textbf{Functional Operation:} CoAP Block-Wise Transfer successfully delivers complete JPEG images (tested with files ranging 50 kB to 5 MB) from leaf nodes to gateway via both direct WiFi and relay store-and-forward paths in 3-node laboratory topology.
    
    \item \textbf{Auto-Role Negotiation:} Bully election successfully promotes highest-priority node to gateway role within 15-30 seconds of boot. Demotion and re-election verified when active gateway is powered off.
\end{enumerate}

\textbf{Estimated / Not Quantified:}

\begin{enumerate}
    \item \textbf{Power Consumption:} Deep sleep idle current estimated at $<$10 µA based on ESP32-S3 and SX1280 datasheet values. No multimeter measurements conducted. Active power draw (WiFi + LoRa + SD) estimated at 150-250 mA but not measured.
    
    \item \textbf{End-to-End Latency:} Total time from timer wake to image transfer completion observed qualitatively via serial logs but not quantitatively benchmarked with timestamps. Transfer duration varies with image size and network conditions.
    
    \item \textbf{Packet Delivery Ratio:} Application-layer retries (3 attempts, 2-second timeout) provide reliability, but PDR percentage not measured across sustained transfers.
    
    \item \textbf{Mesh Recovery Time:} AODV RERR propagation and route re-discovery observed during manual relay power-off, but recovery latency not quantified.
    
    \item \textbf{Multi-Hop Performance:} Relay store-and-forward validated functionally but per-hop latency and throughput degradation not measured.
\end{enumerate}

\subsection{Features Not Implemented in Current Prototype}

The following features were part of the original design proposal but are not implemented in the current prototype:

\begin{enumerate}
    \item \textbf{Python Gateway Dashboard:} The planned laptop-based Python application for real-time image visualization, network health monitoring, and per-node image galleries was not implemented. Images are accessed via physical SD card removal or serial file transfer.
    
    \item \textbf{Queue-Based / TDMA Scheduling:} The gateway-managed transmission queue with time-division slots was not implemented. The current system uses sequential harvest (one node at a time) which provides basic congestion avoidance without formal scheduling.
    ...
    \item \textbf{LoRa DIO1 Hardware Wake:} External interrupt wake via LoRa DIO1 was attempted but proved unreliable due to hardware limitations. Only RTC timer-based wake (180s intervals) is functional.
    
    \item \textbf{Automatic Relay Promotion:} The system does not automatically promote relay nodes when direct Wi-Fi connections fail. Manual relay assignment via HARVEST\_CMD is supported but requires operator intervention.
    
    \item \textbf{Field Testing:} Deployment and testing have been conducted exclusively in laboratory environments. Field testing in simulated off-grid environments is planned as future work.
\end{enumerate}
...
\textbf{[Forecasted — Not Validated in Testing]}

The system architecture supports scalability through its AODV-based routing and sequential harvest approach. The gateway's NodeRegistry supports up to 8 discovered nodes, and the AODV route table accommodates 12 entries with a maximum mesh depth of 3 hops. Adding additional leaf nodes requires no firmware changes as new nodes are automatically discovered via LoRa beacons. However, scaling beyond 8 nodes would require expanding the NodeRegistry capacity. \changed{Only 3-node topologies (1 gateway + 1 relay + 1 leaf) were tested in the laboratory. Multi-hop chains beyond 2 hops, concurrent relay operations, and 8-node deployments were not validated.} Network performance is expected to degrade linearly with additional hops, as each relay node introduces Wi-Fi connection setup latency ($\sim$5--15 seconds per hop) during the harvest cycle, but this has not been quantitatively measured beyond a single relay hop.

\subsection{Key Performance Metrics}

\textbf{[Verified — Laboratory Testing]}

\begin{itemize}
    \item \textbf{Packet Delivery Ratio:} The CoAP Block-Wise Transfer implementation includes application-layer retransmission with 3 retries and a 2-second timeout per block. During laboratory testing of direct (1-hop) image transfers in controlled indoor environment, the system achieves reliable delivery of complete JPEG images (50 kB to 5 MB) across multiple transfer sessions. The Fletcher-16 checksum verification confirms end-to-end data integrity with no observed corruption. \changed{Quantitative PDR percentage not measured.}

    \item \textbf{Data Throughput:} Each CoAP block carries 1024 bytes of image payload within the 1500-byte Wi-Fi MTU limit, preventing IP fragmentation. Pipelined requests with a window of 3 reduce round-trip overhead, improving effective throughput over the baseline sequential approach. \changed{Actual throughput in kbps not benchmarked.}

    \item \textbf{Functional Validation:} Auto-role negotiation via Bully election completes within 15-30 seconds of boot. Gateway failover (manual power-off) triggers successful re-election. CoAP Block2 transfer successfully delivers complete images via both direct WiFi and relay store-and-forward in 3-node laboratory topology.
\end{itemize}

\textbf{[Estimated — Not Measured]}

\begin{itemize}
    \item \textbf{Power Consumption:} Deep sleep with SX1280 in RX mode estimated at $<$10\,$\mu$A idle current based on component datasheets, meeting the $<$100\,$\mu$A target. Active operation (Wi-Fi + LoRa + SD) estimated at 150--250\,mA. \changed{No multimeter measurements conducted over extended sleep/wake cycles. Battery life projections remain unvalidated.}

    \item \textbf{End-to-End Latency:} For a single-hop transfer, latency is dominated by Wi-Fi connection setup ($\sim$5--15 seconds observed in serial logs) plus the CoAP block transfer time. For a 2--5\,MB image at 1024 bytes per block, approximately 2,000--5,000 blocks are required. \changed{Transfer duration observed qualitatively via serial output but not quantitatively measured with timestamps. Estimated 30--90 seconds per image depending on network conditions, but actual timing data not collected.} Multi-hop transfers via relay add additional Wi-Fi connection overhead per hop, but per-hop latency not quantified.
\end{itemize}

\subsection{Comparison with Similar IoT Solutions}

\textbf{[Theoretical Comparison — No Empirical Benchmarking]}

Compared to pure LoRa image transmission systems [10], which occupy the frequency band for several minutes per image, this hybrid approach is designed to complete transfers in under 2 minutes over Wi-Fi while keeping the LoRa channel free for control signaling. \changed{Direct performance comparison with [10] was not conducted — no side-by-side benchmarking of transfer time or spectrum occupancy.}

Compared to standard Wi-Fi mesh solutions that require continuous operation, the implemented sleep-wake architecture reduces idle power consumption by orders of magnitude (estimated $<$10\,$\mu$A vs. typical $\sim$150\,mA for active WiFi mesh). \changed{This power advantage is based on datasheet calculations and has not been validated through actual current measurements or battery lifetime testing.}

The current prototype validates the core data pipeline functionality (CoAP Block2 transfer, AODV routing, auto-role negotiation) but does not provide quantitative scalability or efficiency comparisons with alternative IoT architectures.
...
This project developed a hybrid LoRa + WiFi mesh network prototype that successfully demonstrates the core technical approach for decoupling low-power control signaling from high-bandwidth data transfer on ESP32-S3 microcontrollers. The most important validated finding is that CoAP Block-Wise Transfer (1024-byte blocks) over WiFi, combined with AODV multi-hop routing over LoRa, can reliably transfer JPEG images (2--5 MB) across a 3-node laboratory topology without memory exhaustion.
...
The following capabilities were designed but not implemented or validated in the current prototype:

\begin{enumerate}
    \item \textbf{Security Hardening:} AES-128 LoRa encryption and security penetration testing are required before field deployment.
    
    \item \textbf{Power Consumption Validation:} Quantitative multimeter measurements over extended sleep/wake cycles are needed to validate the estimated $<$10\,$\mu$A idle current and battery life projections.
    
    \item \textbf{Scalability Testing:} Multi-hop chains beyond 2 hops, concurrent relay operations, and deployments with $>$3 nodes require validation.
    
    \item \textbf{Environmental Testing:} Outdoor range, foliage attenuation, temperature extremes, and sustained field deployments remain untested.
    
    \item \textbf{Python Gateway Dashboard:} Real-time image reconstruction and network health monitoring tools are planned for future development.
    
    \item \textbf{Self-Healing Relay Fallback:} Automatic relay promotion when direct WiFi connection fails would improve resilience in sparse topologies.
\end{enumerate}

The implemented system provides a functional proof-of-concept for the hybrid dual-radio architecture. Transitioning from laboratory prototype to deployable wildlife monitoring solution requires the validation and hardening work outlined above.
