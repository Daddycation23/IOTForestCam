...
\textit{Note: The final implementation differs from the originally proposed design in two key areas: (1) ESP-Mesh-Lite for Wi-Fi mesh routing was replaced with AODV routing over the LoRa control plane to enable deep sleep power management, and (2) LoRa DIO1 hardware interrupt wake proved unreliable on the LILYGO T3-S3 V1.2 hardware and was replaced with RTC timer-based wake. These deviations and their justifications are detailed in the Deployment Challenges subsection.}
...
\begin{enumerate}
    ...
    \item \textbf{End-to-End Latency:} \changed{The total time from timer wake to image transfer completion. Measured via serial log timestamps showing wake→announce→harvest→sleep cycle duration.}
...
\changed{Recommended future work includes: (1) developing the Python gateway dashboard for real-time image reconstruction and network health monitoring, (2) adding AES-128 encryption to the LoRa control plane to prevent replay attacks, (3) conducting quantitative power consumption benchmarking in a simulated outdoor deployment environment, and (4) self-healing relay fallback, where a leaf that fails its direct Wi-Fi connection to the gateway falls back to AP mode, allowing the gateway to detect the unreachable node via LoRa beacons and dispatch a relay for store-and-forward harvesting.}
